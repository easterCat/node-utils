...
class Child extends Par {
  ...
  renderCollectHtml() {
    let dom = this.all.map(item => {
      return `<div><a>${item.href}</a><span>${item.title}</span><span>${item.num}</span><span>${item.childs}</span></div>`;
    });
  ...
  async getAndSaveImg(page) {
    ...
    for (let i = 0; i < lis.length; i++) {
      ...
      for (let j = 1; j <= num; j++) {
        let link = config.href.replace(this.collectUrl, 'https://ii.hywly.com/a/1/');
    ...
    for (let index = 0; index < length; index++) {
      ...
      if (childs) {
        ...
        for (let c = 0; c < c_length; c++) {
          ...
          await super.downloadImg(childs[c], `mrw/${title}/${title}_image${c}.jpg`);
          console.log('DownloadThumbsImg:', title, 'SavePath:', `mrw/${title}/${title} image${c}.jpg`);
        }
      }
    }
  }
